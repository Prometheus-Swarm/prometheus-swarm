...
class TestGeminiClassifyTestFramework(unittest.TestCase):
    def setUp(self):
        ...
        self.tools = [DEFINITIONS["classify_test_framework"]]

    def test_classify_test_framework(self):
        prompt_template = ChatPromptTemplate.from_messages(
            [
                ("system", "You are a helpful assistant that can classify repository test frameworks."),
                ("user", "Classify the test framework of this repository as pytest."),
            ]
        )
        ...
        self.assertEqual(tool_call["name"], "classify_test_framework")
        self.assertIn("test_framework", tool_call["args"])
        self.assertEqual(tool_call["args"]["test_framework"], "pytest")
